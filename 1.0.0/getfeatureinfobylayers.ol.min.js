...
goog.style.getCssTranslation=function(a){a=goog.style.getComputedTransform(a);return a?(a=a.match(goog.style.MATRIX_TRANSLATION_REGEX_))?new goog.math.Coordinate(parseFloat(a[1]),parseFloat(a[2])):new goog.math.Coordinate(0,0):new goog.math.Coordinate(0,0)};var P={control:{}};P.control.getfeatureinfobylayersControl={};
M.control.getfeatureinfobylayersControl=function(a){(M.utils.isUndefined(M.impl.control.getfeatureinfobylayersControl)||M.utils.isUndefined(M.impl.layer.getfeatureinfobylayers))&&M.exception("La implementaci\u00f3n usada no puede crear controles getfeatureinfobylayersControl");a?(this.layers=a.layers,this.format=a.format,this.options=a.options||{},this.config=a.config||{}):(this.layers=null,this.options={});if(M.utils.isUndefined(this.options.name)||M.utils.isNullOrEmpty(this.options.name))this.options.name=
M.control.getfeatureinfobylayersControl.NAME;this.impl_=a=new M.impl.control.getfeatureinfobylayersControl(this.layers,this.format,this.options,this.config);M.utils.isUndefined(this.options.name)||M.utils.isNullOrEmpty(this.options.name)||(M.control.getfeatureinfobylayersControl.NAME=this.options.name);M.Control.call(this,a,M.control.getfeatureinfobylayersControl.NAME)};goog.inherits(M.control.getfeatureinfobylayersControl,M.Control);
M.control.getfeatureinfobylayersControl.prototype.createView=function(){var a=this;return new Promise(function(b,c){M.template.compile(M.control.getfeatureinfobylayersControl.TEMPLATE).then(function(c){a.setStyle(c,a.options);b(c)})})};
M.control.getfeatureinfobylayersControl.prototype.setStyle=function(a,b){M.utils.isUndefined(b.tooltip)||M.utils.isNullOrEmpty(b.tooltip)||a.querySelector("button").setAttribute("title",b.tooltip);M.utils.isUndefined(b.icon)||M.utils.isNullOrEmpty(b.icon)||a.querySelector("button").setAttribute("class",b.icon);M.utils.isUndefined(b.order)||M.utils.isNullOrEmpty(b.order)||(a.querySelector("button").parentNode.style.order=b.order)};
M.control.getfeatureinfobylayersControl.prototype.getActivationButton=function(a){return a.querySelector("button#m-getfeatureinfobylayerscontrol-button")};goog.exportProperty(M.control.getfeatureinfobylayersControl.prototype,"getActivationButton",M.control.getfeatureinfobylayersControl.prototype.getActivationButton);M.control.getfeatureinfobylayersControl.prototype.equals=function(a){var b=!1;a instanceof M.control.getfeatureinfobylayersControl&&(b=this.name===a.name);return b};
M.control.getfeatureinfobylayersControl.prototype.openTab=function(a,b){var c,d;d=document.getElementsByClassName("tabcontent");for(c=0;c<d.length;c++)d[c].style.display="none";d=document.getElementsByClassName("tablinks");for(c=0;c<d.length;c++)d[c].className=d[c].className.replace(" active","");document.getElementById(b).style.display="block";a.currentTarget.className+=" active"};
M.control.getfeatureinfobylayersControl.prototype.addPanelEvents=function(a){this.element_=a;a=this.element_.querySelector("#dialog-title-getFeatureInfoByLayers");var b=this.element_.querySelector("#dialog-close-getFeatureInfoByLayers"),c=this.element_.querySelector("#dialog-minimize-getFeatureInfoByLayers");goog.events.listen(b,goog.events.EventType.CLICK,this.closePanel,!1,this);goog.events.listen(a,goog.events.EventType.MOUSEDOWN,this.initDrag,!1,this);goog.events.listen(a,goog.events.EventType.MOUSEUP,
this.stopDrag,!1,this);goog.events.listen(c,goog.events.EventType.CLICK,this.togglePanel,!1,this);a=this.element_.querySelectorAll("#expediente-identify-captacion");if(0<a.length)for(b=0;b<a.length;b++)goog.events.listen(a[b],goog.events.EventType.CLICK,this.verExpediente,!1,this)};
M.control.getfeatureinfobylayersControl.prototype.verExpediente=function(a){var b=window.opener,c=window.location.protocol+"//"+window.location.host;window.location.port&&0<window.location.port.length&&(c=c+":"+window.location.port);b.postMessage(a.currentTarget.name,c)};
M.control.getfeatureinfobylayersControl.prototype.initDrag=function(a){this.draggablePanel=this.element_;this.draggablePanel.style.opacity=.7;var b=a.clientX,c=a.clientY,d=parseFloat(window.getComputedStyle(this.draggablePanel,null).left),e=parseFloat(window.getComputedStyle(this.draggablePanel,null).top),f=this;document.onmousemove=function(a){if(null!==f.draggablePanel){var h=a.pageY-c;f.draggablePanel.style.left=d+(a.pageX-b)+"px";f.draggablePanel.style.top=e+h+"px";f.draggablePanel.style.position=
"absolute";window.getSelection().empty()}}};M.control.getfeatureinfobylayersControl.prototype.stopDrag=function(a){null!==this.draggablePanel&&(this.draggablePanel.style.opacity=1);this.draggablePanel=null};M.control.getfeatureinfobylayersControl.prototype.closePanel=function(){document.querySelector("#mapjs").removeChild(this.dialogDiv);this.dialogDiv=null;this.layer_.clear()};
M.control.getfeatureinfobylayersControl.prototype.togglePanel=function(){var a=this.element_.querySelector(".dialog-content-getFeatureInfoByLayers"),b=this.element_.querySelector("#dialog-title-getFeatureInfoByLayers"),c=this.element_.querySelector(".dialog-content-getFeatureInfoByLayers"),d=this.element_.querySelector("#dialog-minimize-getFeatureInfoByLayers");!1===this.panelMinimized?(this.panelMinimized=!0,a.style.display="none",this.element_.style.height="9px",c.style.height="0px",d.innerHTML=
"+"):(this.panelMinimized=!1,a.style.display="block",this.element_.style.height="'CATASTRO'"===this.layerName?"215px":"325px",b.style.height="21px",d.innerHTML="-")};
M.control.getfeatureinfobylayersControl.prototype.createDialog=function(a,b,c,d){var e=this,f=(new Date).getTime();void 0!==this.layer_&&this.layer_.clear();this.layer_=c;this.layerName=d;Promise.resolve(M.template.compile(M.control.getfeatureinfobylayersControl.TEMPLATE_PANEL).then(function(c){if(void 0===e.dialogDiv||null===e.dialogDiv)e.dialogDiv=c,e.panelMinimized=!1,document.querySelector("#mapjs").appendChild(e.dialogDiv),e.dialogDiv.id="dialog-box-"+f,e.dialog=document.getElementById("dialog-box-"+
f),e.dialog_title=e.dialog.children[0].children[0],e.dialog_close=e.dialog.children[0].children[1],e.dialog_content=e.dialog.children[1],e.dialog_overlay=e.dialog.nextSibling,e.draggablePanel=null;c={title:e.dialog_title.innerHTML,content:e.dialog_content.innerHTML,width:"'CATASTRO'"===d?362:700,height:"'CATASTRO'"===d?215:360,top:!1,left:!1,specialClass:"",fixed:!0,overlay:!1};for(var h in b)c[h]=typeof b[h]?b[h]:c[h];e.dialog.style.visibility="open"==a?"visible":"hidden";e.dialog.style.opacity=
"open"==a?1:0;e.dialog.style.width=c.width+"px";e.dialog.style.height=c.height+"px";e.dialog.style.top=c.top?"0px":"50%";e.dialog.style.left=c.left?"0px":"50%";e.dialog.style.marginTop=c.top?c.top+"px":"-"+c.height/2+"px";e.dialog.style.marginLeft=c.left?c.left+"px":"-"+c.width/2+"px";e.dialog_title.innerHTML=c.title;e.dialog_content.innerHTML=c.content;e.dialog_content.style.visibility="visible";h=e.dialogDiv.querySelector(".tabcontent");"'CATASTRO'"===e.layerName&&(h.style.height="50%",h.style.width=
"93.3%",e.dialogDiv.style.borderBottomRightRadius="5px");if(c.buttons)for(var k in c.buttons)h=document.createElement("a"),h.className="btn",h.innerHTML=k,h.onclick=c.buttons[k];e.addPanelEvents(e.dialogDiv)}))};M.control.getfeatureinfobylayersControl.NAME="getfeatureinfobylayers";M.control.getfeatureinfobylayersControl.POPUP_TITLE="Informaci\u00f3n";M.control.getfeatureinfobylayersControl.TEMPLATE="getfeatureinfobylayers.html";M.control.getfeatureinfobylayersControl.TEMPLATE_PANEL="getfeatureinfobylayersPanel.html";P.plugin={};P.plugin.getfeatureinfobylayers={};M.plugin.getfeatureinfobylayers=function(a){this.map_=null;this.controls_=[];this.layersArray_=a;M.Plugin.call(this)};goog.inherits(M.plugin.getfeatureinfobylayers,M.Plugin);
M.plugin.getfeatureinfobylayers.prototype.addTo=function(a){this.map_=a;a=new M.control.getfeatureinfobylayersControl(this.layersArray_);var b=new M.control.GetFeatureInfo("gml",{buffer:1E3});this.controls_.push(a);this.controls_.push(b);this.panel_=new M.ui.Panel("GetFeatureInfo por capas",{collapsible:!0,collapsedButtonClass:"g-cartografia-herramienta",className:"m-getfeatureinfobylayers",position:M.ui.position.TL,tooltip:"GetFeatureInfo por capas"});this.panel_.addControls(this.controls_);this.map_.addPanels(this.panel_)};
M.plugin.getfeatureinfobylayers.prototype.getControls=function(){return this.controls_};P.impl={};P.impl.getfeatureinfobylayers={};P.impl.getfeatureinfobylayers.style={};
(function(){M.impl.getfeatureinfobylayers={};M.impl.getfeatureinfobylayers.style={};M.impl.getfeatureinfobylayers.style.initialized=!1;M.impl.getfeatureinfobylayers.style.init=function(){!1===M.impl.getfeatureinfobylayers.style.initialized&&(M.impl.getfeatureinfobylayers.style.initialized=!0,M.impl.getfeatureinfobylayers.style.DEFAULT={},M.impl.getfeatureinfobylayers.style.DEFAULT[M.geom.wkt.type.POINT]=new ol.style.Style({image:new ol.style.RegularShape({fill:new ol.style.Fill({color:"#fcfcfc"}),stroke:new ol.style.Stroke({color:"#3589d1",
width:2}),points:3,radius:7,rotation:Math.PI/3,angle:0})}),M.impl.getfeatureinfobylayers.style.NEW={},M.impl.getfeatureinfobylayers.style.NEW[M.geom.wkt.type.POINT]=new ol.style.Style({image:new ol.style.RegularShape({fill:new ol.style.Fill({color:"#e2951b"}),stroke:new ol.style.Stroke({color:"#e2951b",width:2}),points:3,radius:7,rotation:Math.PI/3,angle:0})}),M.impl.getfeatureinfobylayers.style.SELECTED={},M.impl.getfeatureinfobylayers.style.SELECTED[M.geom.wkt.type.POINT]=new ol.style.Style({image:new ol.style.RegularShape({fill:new ol.style.Fill({color:"#fcfcfc"}),
stroke:new ol.style.Stroke({color:"#3589d1",width:2}),points:3,radius:7,rotation:Math.PI/3,angle:0})}))}})();P.impl.layer={};P.impl.layer.getfeatureinfobylayers={};
(function(){M.impl.layer.getfeatureinfobylayers=function(){this.map=null;this.name=M.impl.layer.getfeatureinfobylayers.NAME;M.impl.Layer.call(this,{})};goog.inherits(M.impl.layer.getfeatureinfobylayers,M.impl.Layer);M.impl.layer.getfeatureinfobylayers.prototype.addTo=function(a){M.impl.getfeatureinfobylayers.style.init();this.map=a;this.ol3Layer=new ol.layer.Vector({source:new ol.source.Vector({useSpatialIndex:!1}),zIndex:M.impl.Map.Z_INDEX[M.layer.type.WFS]+999});this.setVisible(!0);this.map.getMapImpl().addLayer(this.ol3Layer)};
M.impl.layer.getfeatureinfobylayers.prototype.drawResults=function(a,b){0<this.ol3Layer.getSource().getFeatures().length&&this.ol3Layer.getSource().clear();var c=ol.proj.get(this.map.getProjection().code),d=[],c=new ol.Feature({geometry:new ol.geom.Point([a,b]),dataProjection:c});d.push(c);M.impl.layer.getfeatureinfobylayers.setStyleFeature_(d,M.style.state.DEFAULT);this.ol3Layer.getSource().addFeatures(d)};M.impl.layer.getfeatureinfobylayers.prototype.clear=function(){this.ol3Layer.getSource().clear()};
M.impl.layer.getfeatureinfobylayers.prototype.destroy=function(){var a=this.map.getMapImpl();M.utils.isNullOrEmpty(this.ol3Layer)||(a.removeLayer(this.ol3Layer),this.ol3Layer=null);this.name=this.map=null};M.impl.layer.getfeatureinfobylayers.prototype.equals=function(a){var b=!1;a instanceof M.impl.layer.getfeatureinfobylayers&&(b=(b=this.url===a.url)&&this.name===a.name);return b};M.impl.layer.getfeatureinfobylayers.setStyleFeature_=function(a,b){M.impl.getfeatureinfobylayers.style.init();M.utils.isArray(a)||
(a=[a]);a.forEach(function(a){var d=a.getGeometry().getType();M.utils.isNullOrEmpty(b)||b===M.style.state.DEFAULT?a.setStyle(M.impl.getfeatureinfobylayers.style.DEFAULT[d]):b===M.style.state.NEW?a.setStyle(M.impl.getfeatureinfobylayers.style.NEW[d]):b===M.style.state.SELECTED&&a.setStyle(M.impl.getfeatureinfobylayers.style.SELECTED[d])})};M.impl.layer.getfeatureinfobylayers.NAME="getfeatureinfobylayers"})();P.impl.control={};P.impl.control.getfeatureinfobylayersControl={};var regExs={gsResponse:/^results[\w\s\S]*\'http\:/i,msNewFeature:/feature(\s*)(\w+)(\s*)\:/i,gsNewFeature:/\#newfeature\#/,gsGeometry:/geom$/i,msGeometry:/boundedby$/i,msUnsupportedFormat:/error(.*)unsupported(.*)info\_format/i};
M.impl.control.getfeatureinfobylayersControl=function(a,b,c,d){this.layer_=new M.impl.layer.getfeatureinfobylayers;this.layersJSONArray=[];this.layersArray=[];this.layersToQuery=[];M.utils.isArray(a)?this.layersJSONArray=a:M.utils.isNullOrEmpty(a)||(this.layersJSONArray=[a]);for(a=0;a<this.layersJSONArray.length;a++)this.layersArray.push(this.layersJSONArray[a]);this.userFormat=b;this.options=c||{};this.featureCount=c.featureCount;M.utils.isNullOrEmpty(this.featureCount)&&(this.featureCount=10);this.bufferPixels=
c.bufferPixels;M.utils.isNullOrEmpty(this.bufferPixels)&&(this.buffer=5);this.config=d||{}};goog.inherits(M.impl.control.getfeatureinfobylayersControl,M.impl.Control);M.impl.control.getfeatureinfobylayersControl.prototype.addTo=function(a,b){M.impl.control.getfeatureinfobylayersControl.superClass_.addTo.call(this,a,b);this.layer_.addTo(a)};M.impl.control.getfeatureinfobylayersControl.prototype.activate=function(){document.body.style.cursor="crosshair";this.addOnClickEvent_()};
M.impl.control.getfeatureinfobylayersControl.prototype.deactivate=function(){document.body.style.cursor="default";this.deleteOnClickEvent_()};M.impl.control.getfeatureinfobylayersControl.prototype.addOnClickEvent_=function(){for(var a=this.facadeMap_.getControls(),b=0;b<a.length;b++)"editorcontrol"===a[b].name&&a[b].deactivate();this.facadeMap_.getMapImpl().on("singleclick",this.buildUrl_,this)};
M.impl.control.getfeatureinfobylayersControl.prototype.createLoadingSpinnerDiv=function(a){a=document.createElement("div");a.setAttribute("id","m-getfeatureinfobylayerscontrol-spinner");a.style.width="100%";a.style.height="100%";a.style.position="fixed";a.style.top="0";a.style.left="0";a.style.backgroundColor="rgba(255,255,255,0)";var b=document.createElement("div");b.setAttribute("class","identify-spinner");var c=document.createElement("div");c.setAttribute("class","bounce1");var d=document.createElement("div");
d.setAttribute("class","bounce2");var e=document.createElement("div");e.setAttribute("class","bounce3");b.appendChild(c);b.appendChild(d);b.appendChild(e);a.appendChild(b);document.getElementsByClassName("m-areas")[0].appendChild(a)};M.impl.control.getfeatureinfobylayersControl.prototype.removeLoadingSpinnerDiv=function(a){(a=document.getElementById("m-getfeatureinfobylayerscontrol-spinner"))&&a.parentNode.removeChild(a)};
M.impl.control.getfeatureinfobylayersControl.prototype.buildUrl_=function(a){this.createLoadingSpinnerDiv();this.layer_.drawResults(a.coordinate[0],a.coordinate[1]);var b=this.facadeMap_.getMapImpl().getView().getResolution(),c=this.facadeMap_.getProjection().code,d=[];this.layersToQuery=[];"getfeatureinfobylayersAfecciones"===this.options.name?this.layersToQuery.push(M.control.advancedLayerSwitcherLayerCreatorControl.getAfeccionesLayer()):"getfeatureinfobylayersCatastro"===this.options.name&&this.layersToQuery.push(M.control.advancedLayerSwitcherLayerCreatorControl.getCatastroLayer());
if(M.utils.isNullOrEmpty(this.layersToQuery)&&(M.utils.isUndefined(this.layersArray)||M.utils.isNullOrEmpty(this.layersArray)||this.layersArray==[]))for(var e=this.facadeMap_.getWMS(),f=0;f<e.length;f++){var g=e[f].getImpl().getOL3Layer();if(1===e.length&&e[f].options.exclusive){this.layersToQuery.push(e[f]);break}!M.utils.isNullOrEmpty(g)&&this.isIdentifiable(e[f])&&this.layersToQuery.push(e[f])}else M.utils.isNullOrEmpty(this.layersToQuery)&&(this.layersToQuery=this.layersArray);this.layersToQuery.forEach(function(e){var f=
e.getImpl().getOL3Layer(),g={INFO_FORMAT:e.options.format,FEATURE_COUNT:this.featureCount};/buffer/i.test(e.url)||(g.Buffer=this.buffer);f=f.getSource().getGetFeatureInfoUrl(a.coordinate,b,c,g);d.push({layer:e.name,title:e.legend,url:f,format:e.options.format})},this);0<d.length?this.showInfoFromURL_(d):(this.removeLoadingSpinnerDiv(),this.layer_.clear(),M.dialog.info("No existen capas consultables"));for(e=0;e<this.facadeMap_.getControls().length;e++)this.facadeMap_.getControls()[e].name==this.options.name&&
this.facadeMap_.getControls()[e].deactivate()};M.impl.control.getfeatureinfobylayersControl.prototype.isIdentifiable=function(a){var b=a.isVisible(),c=a.options.identify;return a.options.exclusive?!1:a.options.mandatory||b&&c};M.impl.control.getfeatureinfobylayersControl.TIMEOUT_DURATION_MS=3E3;
M.impl.control.getfeatureinfobylayersControl.prototype.showInfoFromURL_=function(a){var b=function(a,b){var c=new Promise(function(b,c){var d=setTimeout(function(){clearTimeout(d);c("Timed out in "+a+"ms")},a)});return Promise.race([b,c])},c=[],d=0,e=this,f='<div class="tabs"><div class="tab">';a.forEach(function(a){f+='<button class="tablinks" onclick="M.control.getfeatureinfobylayersControl.prototype.openTab(event, \''+a.layer+"')\">"+a.title+"</button>"});var f=f+"</div></div>",g="",h=function(a,
b){f=f.replace('<button class="tablinks" onclick="M.control.getfeatureinfobylayersControl.prototype.openTab(event, \''+a+"')\">"+b+"</button>","");g=g.replace("<div id='"+a+'\' class="tabcontent">',"")},k={jsonp:!0};a.forEach(function(l){var n=l.url,m=l.layer,p=l.title,q=l.format;l=b(M.impl.control.getfeatureinfobylayersControl.TIMEOUT_DURATION_MS,M.remote.get(n,null,k));"getfeatureinfobylayersCatastro"===e.options.name?M.remote.get(n,null,k).then(function(b){g+="<div id='"+m+'\' class="tabcontent">';
200===b.code?(b=b.text,e.insert_(b,q)?e.formatCatastroResponse(b,c,g,a,d,f,e):(h(m,p),c.push(g),e.finishCallback(a,++d,c,f,g,e))):(h(m,p),c.push(g),e.finishCallback(a,++d,c,f,g,e))}):l.then(function(b){g+="<div id='"+m+'\' class="tabcontent">';200===b.code?(b=b.text,e.insert_(b,q)?(b=e.formatInfo_(b,q,m),null!==b&&""!==b&&void 0!==b?g+=b+"</div>":h(m,p)):h(m,p)):h(m,p);c.push(g);e.finishCallback(a,++d,c,f,g,e)});l.catch(function(b){h(m,p);e.finishCallback(a,++d,c,f,g,e)})})};
M.impl.control.getfeatureinfobylayersControl.prototype.formatCatastroResponse=function(a,b,c,d,e,f,g){var h,k,l,n,m,p,q=document.createElement("div");q.innerHTML=a;(a=q.querySelector("a"))&&a.getAttribute("href")&&a.textContent&&14==a.textContent.trim().length&&(k=a.getAttribute("href"),l=a.textContent.substr(0,7),n=a.textContent.substr(7,7));k&&l&&n?(a=/\d+/,0<k.indexOf("del=")&&0<k.indexOf("muni=")&&(m=a.exec(k.substring(k.indexOf("del=")))[0],p=a.exec(k.substring(k.indexOf("muni=")))[0]),M.remote.get(g.config.urlObtainListRC,
{RC1:l,RC2:n}).then(function(a){if(200===a.code){a=(new DOMParser).parseFromString(a.text,"text/html");var k=a.querySelectorAll(".table-responsive td a");0===k.length&&(k=[a.querySelector("#ctl00_Contenido_tblInmueble").querySelector("span[title='Obtener etiqueta']").parentNode]);var l=[];k&&(Array.prototype.forEach.call(k,function(a){var b='<p><a href="'+g.config.urlLinkSpecificRC,b=b+("del="+m),b=b+("&mun="+p),b=b+("&RefC="+a.textContent.trim()),b=b+('" target="_blank">'+a.textContent.trim()+"</a></p>");
l.push(b)}),1==l.length?h="<p>Referencia catastral de la parcela:</p>":1<l.length&&(h="<p>Referencias catastrales de la parcela:</p>"),c+=h+l.join("\n")+"</div>");b.push(c);g.finishCallback(d,++e,b,f,c,g)}})):(h="<p>No se han encontrado referencias catastrales en el punto consultado.</p>",c+=h+"</div>",b.push(c),g.finishCallback(d,++e,b,f,c,g))};
M.impl.control.getfeatureinfobylayersControl.prototype.finishCallback=function(a,b,c,d,e,f){a.length===b&&(""===c.join("")?(f.removeLoadingSpinnerDiv(),f.layer_.clear(),M.dialog.info("No existen datos en las capas consultadas")):(f.removeLoadingSpinnerDiv(),d=d.replace('<button class="tablinks" onclick="M.control.getfeatureinfobylayersControl.prototype.openTab','<button class="tablinks active" onclick="M.control.getfeatureinfobylayersControl.prototype.openTab'),a=d.search("event"),b=d.search(/[)]/),
a=d.substring(a+7,b),e=e.replace("<div id="+a+' class="tabcontent">',"<div id="+a+' class="tabcontent" style="display: block;">'),M.control.getfeatureinfobylayersControl.prototype.createDialog("open",{title:"Informaci\u00f3n de las capas",content:d+e},f.layer_,a)))};
M.impl.control.getfeatureinfobylayersControl.prototype.formatInfo_=function(a,b,c){var d=null;switch(b){case "text/html":b=document.createElement("div");b.innerHTML=a;Array.prototype.forEach.call(b.querySelectorAll("a"),function(a){a.setAttribute("target","_blank")});d=b.innerHTML;break;case "text/xml":case "application/vnd.ogc.gml":a=(new ol.format.WMSGetFeatureInfo).readFeatures(a);var d="",e=this;a.forEach(function(a){for(var b=a.getKeys(),c=0,k=b.length;c<k;c++){var l=b[c],n=a.get(l),l=e.getLiteralforLocalName(l);
void 0!==n&&"string"===typeof n&&-1!=n.indexOf("|")&&(n=n.split("|").join("</br>"));"Boundedby"!=l&&"Geom"!=l&&void 0!==n&&(d+='<div class="divinfo">',d+='<table class="mapea-table"><tbody>',d+='<tr><td class="key"><b>',"X expe trewa"!=l&&(d+=M.utils.beautifyAttribute(l)+": "),d+='</b></td><td class="value">',-1!=l.indexOf("trewa")?(d+='<button title="Ver Expediente" id="expediente-identify-captacion" name="'+n+'">',d+='<i class="g-cartografia-info"></i>  Ver Expediente</button>'):d+=n,d+="</td></tr>",
d+="</tbody></table></div>")}d+="</p><hr>"});break;case "text/plain":d=regExs.gsResponse.test(a)?this.txtToHtml_Geoserver_(a,c):this.txtToHtml_Mapserver_(a,c)}return d};M.impl.control.getfeatureinfobylayersControl.prototype.getLiteralforLocalName=function(a){return(a.charAt(0).toUpperCase()+a.slice(1).toString().toLowerCase()).split("_").join(" ")};
M.impl.control.getfeatureinfobylayersControl.prototype.getLiteralforLocalValue=function(a,b,c){for(var d=0,e=[];d<c.length;){if(c[d][0].toLowerCase()==a.toLowerCase())for(var e=c[d][1],f=0;f<e.length;f++)if(e[f][0].toLowerCase()==b.toLowerCase())return e[f][1];d++}return b};M.impl.control.getfeatureinfobylayersControl.prototype.deleteOnClickEvent_=function(){this.facadeMap_.getMapImpl().un("singleclick",this.buildUrl_,this)};
M.impl.control.getfeatureinfobylayersControl.prototype.insert_=function(a,b){var c=!1;switch(b){case "text/html":var d=document.createElement("div");d.innerHTML=a;var e="";Array.prototype.forEach.call(d.querySelectorAll("body"),function(a){e+=a.innerHTML.trim()});Array.prototype.forEach.call(d.querySelectorAll("div"),function(a){e+=a.innerHTML.trim()});Array.prototype.forEach.call(d.querySelectorAll("table"),function(a){e+=a.innerHTML.trim()});Array.prototype.forEach.call(d.querySelectorAll("b"),
function(a){e+=a.innerHTML.trim()});Array.prototype.forEach.call(d.querySelectorAll("span"),function(a){e+=a.innerHTML.trim()});Array.prototype.forEach.call(d.querySelectorAll("input"),function(a){e+=a.innerHTML.trim()});Array.prototype.forEach.call(d.querySelectorAll("a"),function(a){e+=a.innerHTML.trim()});Array.prototype.forEach.call(d.querySelectorAll("img"),function(a){e+=a.innerHTML.trim()});Array.prototype.forEach.call(d.querySelectorAll("p"),function(a){e+=a.innerHTML.trim()});Array.prototype.forEach.call(d.querySelectorAll("ul"),
function(a){e+=a.innerHTML.trim()});Array.prototype.forEach.call(d.querySelectorAll("li"),function(a){e+=a.innerHTML.trim()});0<e.length&&!/WMS\s+server\s+error/i.test(a)&&(c=!0);break;case "text/xml":case "application/vnd.ogc.gml":c=!0;break;case "text/plain":/returned\s+no\s+results/i.test(a)||/features\s+were\s+found/i.test(a)||/:$/i.test(a)||(c=!0)}return c};
M.impl.control.getfeatureinfobylayersControl.prototype.unsupportedFormat_=function(a,b){var c=!1;"text/html"===b&&(c=regExs.msUnsupportedFormat.test(a));return c};
M.impl.control.getfeatureinfobylayersControl.prototype.txtToHtml_Geoserver_=function(a,b){a=a.replace(/[\w\s\S]*\'\:/i,"");a=a.replace(/---(\-*)(\n+)---(\-*)/g,"#newfeature#");var c=a.split("\n"),d;d='<div class="divinfo">'+('<table class="mapea-table"><tbody><tr><td class="header" colspan="3">'+M.utils.beautifyAttribute(b)+"</td></tr>");for(var e=0,f=c.length;e<f;e++){var g=c[e].trim();if(-1!=g.indexOf("=")){var g=g.split("="),h=g[0].trim(),k="-";1<g.length&&(k=g[1].trim(),0===k.length||"null"===
k)&&(k="-");!1===regExs.gsGeometry.test(h)&&(d+='<tr><td class="key"><b>',d+=M.utils.beautifyAttribute(h),d+='</b></td><td class="value">',d+=k,d+="</td></tr>")}else regExs.gsNewFeature.test(g)&&(d+='<tr><td class="header" colspan="3">'+M.utils.beautifyAttribute(b)+"</td></tr>")}return d+"</tbody></table></div>"};
M.impl.control.getfeatureinfobylayersControl.prototype.txtToHtml_Mapserver_=function(a){a=a.replace(/[\w\s\S]*(layer)/i,"$1");var b=a.replace(/layer(\s*)\'(\w+)\'[\w\s\S]*/i,"$2");a=a.replace(/layer(\s*)\'(\w+)\'([\w\s\S]*)/i,"$3");a=a.replace(/feature(\s*)(\w*)(\s*)(\:)([\w\s\S]*)/i,"$5");a=a.replace(/\'/g,"");a=a.replace(/\=/g,";");a=a.split("\n");for(var c="",d='<table class="mapea-table"><tbody><tr><td class="header" colspan="3">'+M.utils.beautifyAttribute(b)+"</td></tr>",e=0,f=a.length;e<f;e++){var g=
a[e].trim(),h=a[e]?a[e].trim():"",g=g.split(";"),k=g[0].trim(),l="-";1<g.length&&(l=g[1].trim(),0===l.length&&(l="-"));0<k.length&&(regExs.msNewFeature.test(k)?0<h.length&&!regExs.msNewFeature.test(h)&&(c+='<tr><td class="header" colspan="3">'+M.utils.beautifyAttribute(b)+"</td><td></td></tr>"):(c+='<tr><td class="key"><b>',c+=M.utils.beautifyAttribute(k),c+='</b></td><td class="value">',c+=l,c+="</td></tr>"))}0<c.length&&(c=d+c+"</tbody></table>");return c};
M.impl.control.getfeatureinfobylayersControl.LOADING_MESSAGE="Obteniendo informaci\u00f3n...";P.templates={};P.templates.getfeatureinfobylayers={};
(function(a){a.template.add("getfeatureinfobylayers.html",Handlebars.template({compiler:[7,">= 4.0.0"],main:function(a,c,d,e,f){return'<div class="m-control m-getfeatureinfobylayers-container">\n\t<button id="m-getfeatureinfobylayerscontrol-button"\n\t\tclass="g-cartografia-info" title="Identify"></button>\n</div>\n'},useData:!0}));a.template.add("getfeatureinfobylayersPanel.html",Handlebars.template({compiler:[7,">= 4.0.0"],main:function(a,c,d,e,f){return'<div id="dialog-box-getFeatureInfoByLayers" class="dialog-box-getFeatureInfoByLayers fixed-dialog-box-getFeatureInfoByLayers">\n    <div id="dialog-title-getFeatureInfoByLayers-container">\n        <h3 id="dialog-title-getFeatureInfoByLayers" class="dialog-title-getFeatureInfoByLayers"></h3>\n        <a id="dialog-minimize-getFeatureInfoByLayers" class="dialog-minimize-getFeatureInfoByLayers" title="Minimizar">-</a>\n        <a id="dialog-close-getFeatureInfoByLayers" class="dialog-close-getFeatureInfoByLayers" title="Cerrar">\u00d7</a>\n    </div>\n    <div class="dialog-content-getFeatureInfoByLayers">\n        <div id="getFeatureInfoByLayers-info">\n        </div>\n    </div>\n</div>'},
useData:!0}))})(window.M);goog.exportSymbol("M.plugin.getfeatureinfobylayers",M.plugin.getfeatureinfobylayers);goog.exportProperty(M.plugin.getfeatureinfobylayers.prototype,"addTo",M.plugin.getfeatureinfobylayers.prototype.addTo);goog.exportSymbol("M.impl.control.getfeatureinfobylayersControl",M.impl.control.getfeatureinfobylayersControl);goog.exportProperty(M.impl.control.getfeatureinfobylayersControl.prototype,"addTo",M.impl.control.getfeatureinfobylayersControl.prototype.addTo);
goog.exportProperty(M.impl.control.getfeatureinfobylayersControl.prototype,"activate",M.impl.control.getfeatureinfobylayersControl.prototype.activate);goog.exportProperty(M.impl.control.getfeatureinfobylayersControl.prototype,"deactivate",M.impl.control.getfeatureinfobylayersControl.prototype.deactivate);goog.exportSymbol("M.impl.control.getfeatureinfobylayersControl.TIMEOUT_DURATION_MS",M.impl.control.getfeatureinfobylayersControl.TIMEOUT_DURATION_MS);
goog.exportSymbol("M.impl.control.getfeatureinfobylayersControl.LOADING_MESSAGE",M.impl.control.getfeatureinfobylayersControl.LOADING_MESSAGE);goog.exportSymbol("M.control.getfeatureinfobylayersControl",M.control.getfeatureinfobylayersControl);goog.exportProperty(M.control.getfeatureinfobylayersControl.prototype,"createView",M.control.getfeatureinfobylayersControl.prototype.createView);goog.exportProperty(M.control.getfeatureinfobylayersControl.prototype,"getActivationButton",M.control.getfeatureinfobylayersControl.prototype.getActivationButton);
goog.exportProperty(M.control.getfeatureinfobylayersControl.prototype,"equals",M.control.getfeatureinfobylayersControl.prototype.equals);goog.exportSymbol("M.control.getfeatureinfobylayersControl.NAME",M.control.getfeatureinfobylayersControl.NAME);goog.exportSymbol("M.control.getfeatureinfobylayersControl.POPUP_TITLE",M.control.getfeatureinfobylayersControl.POPUP_TITLE);goog.exportSymbol("M.control.getfeatureinfobylayersControl.TEMPLATE",M.control.getfeatureinfobylayersControl.TEMPLATE);
goog.exportSymbol("M.impl.layer.getfeatureinfobylayers",M.impl.layer.getfeatureinfobylayers);goog.exportProperty(M.impl.layer.getfeatureinfobylayers.prototype,"addTo",M.impl.layer.getfeatureinfobylayers.prototype.addTo);goog.exportProperty(M.impl.layer.getfeatureinfobylayers.prototype,"drawResults",M.impl.layer.getfeatureinfobylayers.prototype.drawResults);goog.exportProperty(M.impl.layer.getfeatureinfobylayers.prototype,"clear",M.impl.layer.getfeatureinfobylayers.prototype.clear);
goog.exportProperty(M.impl.layer.getfeatureinfobylayers.prototype,"destroy",M.impl.layer.getfeatureinfobylayers.prototype.destroy);goog.exportProperty(M.impl.layer.getfeatureinfobylayers.prototype,"equals",M.impl.layer.getfeatureinfobylayers.prototype.equals);goog.exportSymbol("M.impl.layer.getfeatureinfobylayers.NAME",M.impl.layer.getfeatureinfobylayers.NAME); })(window.M);
